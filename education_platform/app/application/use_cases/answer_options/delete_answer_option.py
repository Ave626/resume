    ...
    async def execute(self, command: DeleteAnswerOptionCommand) -> None:
        async with self.uow:
            ...
            if answer_option is None:
                raise AnswerOptionNotFoundError("AnswerOption not found")

            question = await self.uow.questions.get_by_id(answer_option.question_id)
            if question is None:
                raise QuestionNotFoundError("Question not found error")
            ...
            question.validate_answer_configuration(remaining_options)
