...
async def get_courses(use_case : GetCoursesUseCase = Depends(get_get_courses_use_case)) -> list[CourseListItemResponse]:
...
async def get_course(course_id : UUID,use_case : GetCourseUseCase = Depends(get_get_course_use_case)) -> CourseResponse:
    result = await use_case.execute(GetCourseQuery(course_id=course_id))
    return CourseResponse.model_validate(result)

@router.get("/course/{course_id}/structure",response_model=CourseStructureResponse)
